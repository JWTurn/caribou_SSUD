...
make_pde <- function(mod, lsRasters, studyArea, normalize = TRUE) {
  ...
  numerator <- lapply(
    1:nrow(selectionCoefs),
    function(x) {
      2 * as.double(selectionCoefs$estimate[[x]]) *
        eval(parse(text = selectionCoefs$expr[[x]]),
             envir = lsRasters)
    }
  )
  ...
  U <- exp(app(numeratorRast, fun = 'sum', na.rm = TRUE))
